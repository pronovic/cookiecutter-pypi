...
**Import**) the `{{ cookiecutter.project_slug }}` directory in PyCharm, then follow the remaining
...
Go to the PyCharm settings and find the `{{ cookiecutter.project_slug }}` project.  Under
**Project Structure**, mark `src` as a source folder and `tests` as a test
folder.  In the **Exclude Files** box, enter the following:

```
LICENSE;NOTICE;PyPI.md;build;dist;docs/_build;out;poetry.lock;poetry.toml;run;.coverage;.coverage.lcov;.coveragerc;.gitattributes;.github;.gitignore;.htmlcov;.idea;.mypy_cache;.poetry;.pre-commit-config.yaml;.pytest_cache;.readthedocs.yml;.ruff_cache;.run;.tabignore;.venv
```
...
Documentation at [Read the Docs](https://{{ cookiecutter.project_slug }}.readthedocs.io/en/stable/)
is generated via a GitHub hook.  So, there is no formal release process for the
documentation.

### Code

Code is released to [PyPI](https://pypi.org/project/{{ cookiecutter.project_slug }}/).  There is a
